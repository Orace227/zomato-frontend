...
export default function Dropdown(props) {
    ...
    return (
        <>
            <Menu as="div" className={`md:absolute right-44 border ml-3 rounded-md inline-block text-left ${props.number1 == 24 ? '' : 'hidden'}`}>
                <div >
                    <Menu.Button className="w-[170px] inline-flex justify-center  gap-x-1.5 rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50">
                        {Title}
                        <ChevronDownIcon className="relative top-[1px] h-5 w-5 text-gray-400" aria-hidden="true" />
                    </Menu.Button>
                </div>
            ...
            <Menu as="div" className={`md:absolute md:right-16 inline-block border rounded-md  text-left ${props.number2 == 12 ? '' : 'hidden'}`}>
                <div>
                ...
                <Transition
                    ...
                    leaveTo="transform opacity-0 scale-95"
                >
                    <Menu.Items className="md:absolute right-0 z-10 mt-2 w-[160px] origin-top-right rounded-md bg-white shadow-lg   ring-1 ring-black ring-opacity-5 focus:outline-none">
                        <div className={`py-1 flex flex-col`}>
